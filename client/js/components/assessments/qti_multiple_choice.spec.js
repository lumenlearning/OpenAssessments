...
describe('qti multiple choice', function() {
  ...
  it('renders the radio button label', function() {
    expect(result.getDOMNode().textContent).toContain(items[0].material);
  });
  
  it('renders input attributes', function() {
    expect(result.getDOMNode().childNodes[0].childNodes[0].childNodes[0].attributes.name.value).toContain(items[0].name);
  });
